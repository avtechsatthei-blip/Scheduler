 ...
 * Presentation 2), each its own PNG (and optionally its own BrightSign file). */
(function (root) {
  ...
  SA.open = (ctx) => {
    ...
    const rows = [
      { time: ctx.event.start || '', title: ctx.spec.name, themeKey: ctx.fallbackKey },
      { time: U.fromMin(Math.min(1439, U.toMin(ctx.event.start || '09:00') + 60)), title: '', themeKey: ctx.fallbackKey },
    ];
    let results = [];

    const rowHtml = (r, i) => `<div class="row wrap" data-ri="${i}" style="gap:8px;align-items:flex-end">
        ...
        ${UI.field('Design', `<select class="in sm" data-r="themeKey">${all.map((t) => UI.opt(t.key, t.label, t.key === r.themeKey)).join('')}</select>`)}
    ...
    function redrawRows() { UI.$('#sa-rows', m.el).innerHTML = rows.map(rowHtml).join(''); }

    function wire() {
      m.el.addEventListener('input', (e) => {
        const row = e.target.closest('[data-ri]'); const f = e.target.dataset.r;
        if (row && f) rows[+row.dataset.ri][f] = e.target.value;
      });
      ...
      m.el.addEventListener('click', async (e) => {
        if (e.target.closest('#sa-add')) { rows.push({ time: '', title: '', themeKey: ctx.fallbackKey }); redrawRows(); }
    ...
    async function generate() {
      ...
      for (const r of valid) {
        const th = Sign.resolveTheme(st, r.themeKey);
        await Sign.preload(th);
        const spec = { name: r.title.trim(), room: ctx.spec.room };
        const canvas = Sign.render(spec, { theme: th, showDate: false });
