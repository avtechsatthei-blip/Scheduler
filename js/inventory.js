    ...
    Object.entries(usage).forEach(([k, u]) => {
      ...
      Object.entries(u.byDate).forEach(([date, p]) => {
        ...
        if (!inv) {
          ...
          worst = 'short';
          alerts.push({ level: 'short', kind: 'short', item: u.name, date, peak: p.peak, have, need, when, events: evNames, rentable: inv.rentable, rentCost: cost, text: `Short ${need} ${u.name}${need > 1 ? 's' : ''} on ${U.fmtDay(date)}: ${p.peak} in use at ${when}, ${have} on hand.` + (inv.rentable ? ` Rent ${need}${cost ? ' (about $' + cost + ')' : ''}.` : '') });
      ...
      rows.push({ key: k, name: u.name, have, inInventory: !!inv, weekPeak, weekPeakDate, status: worst, byDate: Object.fromEntries(Object.entries(u.byDate).map(([d, p]) => [d, p.peak])) });
    });
    // items in inventory that aren't used this week
    state.inventory.forEach((i) => {
      if (!usage[U.itemKey(i.name)]) rows.push({ key: U.itemKey(i.name), name: i.name, have: +i.qty, inInventory: true, weekPeak: 0, weekPeakDate: null, status: 'idle', byDate: {} });
    });
