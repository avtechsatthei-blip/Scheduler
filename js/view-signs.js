/* Room signs: preview and download PNGs (single or ZIP), with a shared theme library plus per-sign overrides */
(function (root) {
  const IH = root.IH, U = IH.U, UI = IH.UI, Store = IH.Store, Sign = IH.Sign;
  const { esc, icon: ic } = UI;
  const mem = (UI.mem.signs = { scope: 'week', theme: null, showDate: null, merge: true, sel: new Set(), custom: { name: '', room: '', override: null }, current: null });
    ...
    render() {
      ...
      return top + UI.filterBar({ days: mem.scope === 'week' ? 'week' : 'date' }) + `<div class="row wrap noprint" style="margin-bottom:16px;gap:12px">
        ...
        <div class="signwrap"><div>
          ...
          }).join('') : '<div class="empty" style="padding:30px 16px"><p style="margin:0">No events in this view. Add some, or make a custom sign.</p></div>'}
        ...
        <div><div id="cust" class="${mem.current === '__custom' ? '' : 'hide'} card pad" style="margin-bottom:14px"><div class="form-grid">
          ...
          <div class="row wrap" style="margin-top:12px"><span class="muted small grow" id="pv-name"></span><button class="btn" data-act="sign-customize" data-key="${esc(mem.current)}">${ic('edit', 'sm')} Customize this sign</button><button class="btn primary" data-act="sign-dl">${ic('download', 'sm')} Download PNG</button></div>
  ...
  async function paint(ent, box) {
    ...
    const c = Sign.render(r.spec, { theme: r.theme, showDate: showDate() && !ent.custom });
    box.innerHTML = '';
    box.appendChild(c);
    return c;
  }
  ...
  const toBlob = async (ent) => { const r = resolved(ent); await Sign.preload(r.theme); const c = Sign.render(r.spec, { theme: r.theme, showDate: showDate() && !ent.custom }); return Sign.toBlob(c); };
