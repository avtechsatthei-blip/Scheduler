...
You can also drop in a **.zip** of PDFs instead of picking files one at a time — handy if you keep a folder per week with a subfolder per day or event. It's unpacked automatically, at any folder depth, and any folder whose name looks like an old or superseded set of plans ("Old Plans", "Archive", "Superseded", "Outdated", "Obsolete", "Deprecated", whole-word matches only) is skipped, folder and everything inside it. The review screen tells you which folders it left out.

## Room signs and PowerPoint

- **Room signs** makes 1920x1080 PNGs (event name centered, room name along the bottom). *Classic gray* matches the example sign; *iHotel blue and orange* is the brand version; *Hotel Illinois (U of I)* adds the Hotel Illinois Conference Center logo across the top. Download one, or tick several for a ZIP. A day picker (with a one-click *Tomorrow*) narrows the list to a single day.
- **Your own designs**: *Manage designs* (in Room signs or Settings) opens a small library. *New design* lets you upload a logo, then *Match colors to logo* samples the image and fills in a matching palette (room bar, accent strip, title and room text) that you can fine-tune with the color pickers or by typing a hex code. *Logo size* and *Logo position*, and *Text size* and *Text position*, let you resize and reposition the logo and the event-name text independently — handy for a wide or tall logo, or to make room for a longer title. *Crop* opens a drag-to-move, drag-corner-to-resize tool for trimming the logo image itself (e.g. cutting out extra whitespace); it always starts from your original upload, so cropping again later never loses quality. A separate **Background image** can be added too — it fills the whole sign, cropped to fit, behind everything else (the Background color is unused once one is set); this shows up in both the PNG and the PowerPoint export. Saved designs show up next to the built-in ones everywhere a sign style is picked, and can be edited, duplicated or deleted later.
...
- **Advanced (the clock icon on a sign)**: build several time-of-day versions of one event's sign — e.g. 10:00 "Presentation 1", 11:00 "Presentation 2" — each with its own text and, if you like, its own design (its own logo, colors, size and position — pick "Custom for this time" on any row). Generates a PNG per time, downloadable individually, as a ZIP, or as a BrightSign ZIP (a `.bpfx` + `.PNG` pair for each time).
- **Bulk BrightSign**: tick several signs in the list and *Download selected for BrightSign* builds one zip with a `.bpfx` + `.PNG` pair for each of them.
- **BrightSign**: the *BrightSign* button next to a sign's download builds a real BrightAuthor:connected project file (`.bpfx`) for that sign, zipped together with its PNG. A `.bpfx` never embeds the image — it just points at a file on disk — so unzip both files into the right folder before opening it in BrightAuthor. That folder is **Settings, BrightSign asset folder** plus **Settings, Dated subfolder** (default `{YYYY}\{MM} {Month}\{DD}\{M}.{DD}`, filled in from the sign's event date) — Settings shows a live example of the exact path. If BrightAuthor can't find the file, this is the first thing to check: the PNG has to be sitting in that exact dated folder. The generated file reuses your player's real hardware profile (captured from a working XT1144 export), so it opens as a normal single-image presentation ready to publish.
