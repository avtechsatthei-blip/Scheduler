...
- **BrightSign**: the *BrightSign* button next to a sign's download builds a real BrightAuthor:connected project file (`.bpfx`) for that sign, zipped together with its PNG. A `.bpfx` never embeds the image — it just points at a file on disk — so unzip both files into the folder set in **Settings, BrightSign asset folder** (or wherever you keep signage images) before opening it in BrightAuthor. The generated file reuses your player's real hardware profile (captured from a working XT1144 export), so it opens as a normal single-image presentation ready to publish.
