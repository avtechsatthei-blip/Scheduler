...
- **Advanced (the clock icon on a sign)**: build several time-of-day versions of one event's sign — e.g. 10:00 "Presentation 1", 11:00 "Presentation 2" — each with its own text and, if you like, its own design. Generates a PNG per time, downloadable individually, as a ZIP, or as a BrightSign ZIP (a `.bpfx` + `.PNG` pair for each time).
